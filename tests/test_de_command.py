...
def test_de_delete_to_end_of_word():
    ...
    # Execute d then e
    result = OperatorMotionHandler.execute_operator_motion(
        widget, 'd', widget.nav_word_end, 1
    )
    ...
    assert len(widget.text) < len(initial_text), f"Text should be shorter. Was '{initial_text}', now '{widget.text}'"
